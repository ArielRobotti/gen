    ...
    <header className="w-full flex justify-between items-center p-1 bg-gray-900 text-white shadow-md">
      ...
      <div
        className={`fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 transition-opacity duration-1000 
          ${showModalRegister ? 'opacity-85 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setShowModalRegister(false)}
      >
        <div
          onClick={(e) => e.stopPropagation()}
          className={`bg-white text-black p-6 rounded-lg shadow-lg transform transition-all duration-1000 
              ${showModalRegister ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
        >
          <h2 className="font-semibold text-lg mb-2">Register User</h2>
